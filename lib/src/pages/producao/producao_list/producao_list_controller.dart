...
abstract class _ProducaoListControllerBase with Store {
  ...
  Future setProducaoItemSaida(ItemEstoque model, Producao producao) async {
    ...
    for (var prod in producaoItensDtNovoEntrada) {
      if (pi.vlUnit == null) {
        pi.vlUnit =
            pi.calculaVlTotal(prod.qt.toString(), prod.vlUnit.toString());
      } else {
        pi.vlUnit = pi.vlUnit +
            pi.calculaVlTotal(prod.qt.toString(), prod.vlUnit.toString());
      }
    ...
    if (producao.vlCustosInd != null) {
      pi.vlUnit = pi.vlUnit + producao.vlCustosInd;
    }

    if (producao.vlMo != null) {
      pi.vlUnit = pi.vlUnit + producao.vlMo;
    }

    //pi.vlUnit = pi.vlUnit / 1000;
  ...
  @action
  Future<bool> verificaVlMinEstoque(
      {int idItem,
      ProducaoItem producaoItem,
      ProducaoListController producaoListController}) async {
    ItemDAO itemDAO = ItemDAO();
